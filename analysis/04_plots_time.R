...
enzyme_plot <- ggplot(ER_data_long, aes(x = sample_date, y = median_value, group = position, color = position, shape = position)) +
...
enzyme_all <- enzyme_plot +
  geom_text(data = facet_labels, aes(x = levels(ER_data_long$sample_date)[4],  # Leftmost position
                                     y = Inf, label = label),
                                     inherit.aes = FALSE, hjust = 0.2,  vjust = 1.7, size = 4)
